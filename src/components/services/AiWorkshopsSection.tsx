...
interface AiWorkshopsSectionProps {
  theme: Theme;
}

export const AiWorkshopsSection: React.FC<AiWorkshopsSectionProps> = ({ theme }) => {
  ...
  return (
    <section id="s2" className={`py-16 md:py-24 transition-colors duration-300 ${theme === Theme.DARK ? 'bg-black text-white' : 'bg-gray-100 text-gray-900'}`}>
      <div className="container mx-auto px-6">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-4">AI Workshops & Team Training</h2>
        <p className={`text-lg md:text-xl text-center max-w-3xl mx-auto mb-12 ${theme === Theme.DARK ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <div className="grid md:grid-cols-2 gap-x-10 gap-y-8 mb-12">
          <div className={`${theme === Theme.DARK ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-md`}>
            ...
            <ul className="space-y-1.5">
              {workshopFormat.map((item, index) => (
                <li key={index} className={`text-sm ${theme === Theme.DARK ? 'text-gray-300' : 'text-gray-700'}`}>{item}</li>
              ))}
            </ul>
          </div>
          <div className={`${theme === Theme.DARK ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-md`}>
            ...
            <p className={`text-sm ${theme === Theme.DARK ? 'text-gray-300' : 'text-gray-700'}`}>{coveredTools.join(', ')}</p>
          </div>
          <div className={`${theme === Theme.DARK ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-md`}>
            ...
            <ul className="space-y-1.5">
              {levels.map((level) => (
                <li key={level.name} className={`text-sm ${theme === Theme.DARK ? 'text-gray-300' : 'text-gray-700'}`}>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6">
          <Link
            to="/workshop"
            className={`inline-flex items-center justify-center font-semibold py-3 px-6 rounded-lg text-md transition-transform duration-300 ease-in-out hover:scale-105 shadow-md ${buttonTextColor}`}
